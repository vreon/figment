...
def emote(action_name, actor, verb, plural=None, join=None, selector=None):
    ...
    if not unique_selection(actor, action_name, 'selector', selector, kwargs, targets, 'nearby'):
        return
...
def eyebrow(actor, join=None, selector=None):
    return emote('eyebrow', actor, 'raise an eyebrow', 'raises an eyebrow', join='at', selector=selector)
